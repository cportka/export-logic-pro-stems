...
for f in \
  docs/index.html docs/css/style.css docs/js/stem-lib.js docs/js/app.js docs/js/companion.js docs/.nojekyll \
  ...
  scripts/extract-dry-stems.py scripts/bounce-wet-stems.sh scripts/bounce-wet-stems.applescript \
  companion/stem-companion.py; do
  need "$f"
done
...
if command -v node >/dev/null 2>&1; then
  ...
  node --check docs/js/companion.js || fail=1
...
if command -v python3 >/dev/null 2>&1; then
  ...
  python3 -m py_compile companion/stem-companion.py || fail=1
fi
